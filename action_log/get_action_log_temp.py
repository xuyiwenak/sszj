                ...
                try:
                    ...
                    for table_row in cur_logdb.fetchall():
                        ...
                        log_cur.execute("select char_id, mapid, level, log_type, param1, param2, param3, param4, log_time, str_param, %s\
                                    from %s where log_time between unix_timestamp('%s') and unix_timestamp('%s') and log_type in (55,56,57) " \
                                    % ( gateway_id, table_name,time_start,time_end))
                        log_data = log_cur.fetchall()
                        #print log_data
                        dest_sql = "insert into log_action values(%s, %s, %s, %s, %s ,%s, %s, %s ,%s, %s, %s)"
...
online_data2db("103.244.235.249", 'sszj', 'DR9m_wqsgF8a', 'data_stat_ss','conf_server_list_2','action_log','2017-09-22 00:00:00','2017-09-29 15:10:00')
