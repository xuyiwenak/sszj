...
def online_data2db(DB_HOST, DB_USER, DB_PWD, db_name, DB_NAME):
    ...
    try:
        ...
        dbsql = "select log_name,log_host_ip, gateway_id from conf_server_list_vn"
        cur.execute(dbsql)
        for row in cur.fetchall():
            ...
            if 1 == 1:
                ...
                try:
                    row_conn = MySQLdb.connect(host = dbhost, user = DB_USER, passwd = 'F6TAzcbvY5Gpi', db = dbname, port = 3306)
                    ...
                    for table_row in cur_logdb.fetchall():
                        ...
                        bool_in_range=compare_time(date,'2017-05-01 00:00:00','2017-06-30 23:59:59')
                        if  bool_in_range == False:
                            continue
                        print "get table name  %s , gatewayid %s \n" %(table_name,gateway_id)
                        log_cur.execute("select map_id, char_id, char_level, total_damager, totoal_injured, type, creature_killed, money_gained,death_times, exp_gained, op_type ,log_time,%s\
                                    from %s where log_time between unix_timestamp('2017-05-01 00:00:00') and unix_timestamp('2017-06-30 23:59:59')" % ( gateway_id, table_name))
                        log_data = log_cur.fetchall()
                        #print log_data
                        dest_sql = "insert into activity_log values(%s, %s, %s, %s, %s, %s ,%s, %s, %s ,%s, %s, %s, %s)"
...
online_data2db("103.244.235.249", 'sszj', 'DR9m_wqsgF8a', 'data_stat_ss', 'data_stat_ss')
