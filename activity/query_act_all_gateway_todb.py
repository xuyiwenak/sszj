...
filename = "/data/home/sszj/data_stat/activity/activity_allgateway.txt"
f = open(filename, "w")
...
def query_per_day(DB_HOST, DB_USER, DB_PWD, DB_NAME ,time_start,time_end):
    try:
        ...
        while time_post < s_time_end:
            ...
            print show_time
            cur.execute("select a.map_id, count, b.value, a.gateway_id from \
                                (select * from (select map_id, count(role_id) as count, gateway_id from log_activity where op_type =1 and log_time between %s and %s group by gateway_id, map_id)\
            ...
            for row in cur.fetchall():
                ...
                value = row[2]
                gateway_id = row[3]
                activity_day_log_str = "%d,%d,%d,%s,%s" %(gateway_id, map_id,count,value,show_time)
