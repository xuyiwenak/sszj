...
def query_per_day(DB_HOST, DB_USER, DB_PWD, DB_NAME ,time_start,time_end):
    try:
        ...
        while time_post < s_time_end:
            ...
            print show_time
            cur.execute("select gatewayid,map_id,count(char_id) from activity_log where op_type =1 and log_time between %s and %s group by gatewayid,map_id" %(time_pre ,time_post)) 
            for row in cur.fetchall():
                gateway_id = row[0]
                map_id = row[1]
                count_char_id = row[2]
                activity_day_log_str = "%d,%d,%d,%s" %(gateway_id,map_id,count_char_id,show_time)
