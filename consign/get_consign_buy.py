...
def query_role(db_host, db_user, db_passwd, db_name, db_port, gateway_id, game_id):
    ...
    try:
      ...
      sql = "select * from (select sheet_name, sum(quantity) as count, sum(price) as total_price, (sum(price) / sum(quantity)) as single_price from gms_consign_log where consign_flag = 1 and from_unixtime (consign_time) >= '2017-06-08 00:00:00' group by sheet_name) b order by count desc"
      cur.execute(sql)
      for row in cur.fetchall():
         ...
         total_price = row[2]
         single_price = row[3]
         dest_sql = "insert into consign_buy values('%s', %d, %d, %f)" % (sheet_name, count, total_price, single_price)
