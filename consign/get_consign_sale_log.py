...
def query_role(db_host, db_user, db_passwd, db_name, db_port, gateway_id, game_id):
    ...
    try:
      ...
      sql = "select sheet_name, quantity, price, add_time from gms_consign where consign_flag = 0 and from_unixtime(add_time) >= '2017-05-12 00:00:00'"
      cur.execute(sql)
      for row in cur.fetchall():
         ...
         consign_flag = 0
         consign_time = row[3]
         dest_sql = "insert into consign_log values('%s', %d, %d, %d, %d, %d)" % (sheet_name, count, price, consign_flag, consign_time, gateway_id)
